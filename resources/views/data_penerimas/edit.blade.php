    ...
    <form method="post" action="{{ route('data_penerimas.update', $data_penerimas->id) }}">
        ...
        <div class="form-group">
            ...
            <div class="form-check form-check-primary mt-3">
                <input name="status" class="form-check-input" type="radio" value="DITERIMA" id="status_diterima" {{ $data_penerimas->status === 'DITERIMA' ? 'checked' : '' }}>
                <label class="form-check-label" for="status_diterima"> DITERIMA </label>
            </div>
            <div class="form-check form-check-secondary">
                <input name="status" class="form-check-input" type="radio" value="DIKEMBALIKAN" id="status_dikembalikan" {{ $data_penerimas->status === 'DIKEMBALIKAN' ? 'checked' : '' }}>
